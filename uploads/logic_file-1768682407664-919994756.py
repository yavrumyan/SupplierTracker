...
- ROUNDS PRICES to 2 decimal places (preserves cents).
"""
...
def standardize(input_path: str, output_path: str, include_no_price: bool = True):
    ...
    if not target_sheet:
        ...
        pd.DataFrame(columns=["Supplier","Category","Brand","Model","Name","Price","Currency","Stock","MOQ","Notes"]).to_csv(output_path, index=False)
        return
