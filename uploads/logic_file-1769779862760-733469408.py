...
- Mapping:
    ...
    - Col D = Stock
    - Col E = Price (USD)
    - Col F = MOQ (Mapped to 'MOQ' column)
...
def standardize(input_path: str, output_path: str):
    print("Starting HUBX conversion (MOQ from Col F)...")
    ...
    for sheet_name in wb.sheetnames:
        ...
        # We keep F (5) here too so it appears in notes for context
        ...
        for row in ws.iter_rows(min_row=start_row, values_only=False):
            ...
            clean_moq = "1" # Default
            if raw_moq:
                # If the cell contains just a number (e.g. 5 or "5"), use it.
                # If it says "5+" or "MOQ 5", we keep it as text in the MOQ field 
                # or you can choose to strip it. Here we keep the raw text which is safer.
                clean_moq = raw_moq

            # 6. MODEL
            final_model = clean_text(get_val(idx_model))

            # 7. NOTES (Merge multiple columns)
            notes_parts = []
            for n_idx in idx_notes_list:
                val = clean_text(get_val(n_idx))
                if val and val.lower() != "nan":
                    # Add labels for clarity? Or just values?
                    # Previous logic was just values joined by pipe.
            ...
            products.append({
                ...
                "Currency": "USD", 
                "Stock": clean_stock,
                "MOQ": clean_moq,  # Populated from Col F
                "Notes": final_notes
            })
