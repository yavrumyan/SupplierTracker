...
def standardize(input_path: str, output_path: str):
    ...
    for i, row in df.iterrows():
        ...
        # Extract Raw Values (Indices based on your description)
        # Col 0-3: Name parts
        # Col 4: Price
        # Col 5-7: Notes parts (Warr, Keyb, etc)
        ...
        # If the Price column literally says "Price", this is a Header Row.
        # The Category Name is in the first column.
        ...
        # Skip empty rows or rows with no price info
        if not raw_price or raw_price == "":
            continue
            
        # Skip rows that are clearly just info (like dates or phone numbers in the price column)
        # Real prices are digits, or "Call". If it contains "2025" it might be a date.
        # Simple check: If it contains ":" or "@" it is likely contact info, not a price.
        ...
        for idx in [5, 6, 7]:
            if idx < len(row) and pd.notna(row[idx]):
                val = str(row[idx]).strip()
                # Skip the column header text if it appears (like "Warr.")
        ...
        products.append({
            ...
            "Price": clean_price,
            "Currency": "USD",
            "Stock": "0", # No stock column specified
            "MOQ": "NO",
            "Notes": full_notes
        })
