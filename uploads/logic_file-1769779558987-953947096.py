...
def standardize(input_path: str, output_path: str):
    print("Starting PHONIX conversion (MOQ from Col F)...")
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=start_row, values_only=False):
            ...
            clean_moq = "1" # Default
            if raw_moq:
                # If it's a simple number, use it. If text like "10+", keep as is or parse?
                # Usually MOQ field expects a number or simple string.
                clean_moq = raw_moq
            ...
            # 7. NOTES (Complex Merge)
            # Structure: MOQ {F} | ETA {G} | Conditions {I} | {J}
            notes_parts = []
            
            if raw_moq: notes_parts.append(f"MOQ {raw_moq}")
